	...
	/*	SimpleMailMessage message = new SimpleMailMessage();
     ...
     /*   MimeMessage messageHtml = emailSender.createMimeMessage();
      ...
      /*  MimeMessage messageFile = emailSender.createMimeMessage();
		...
		for(int i=0; i<listCategorie.length; i++) {
			...
			CategoryProject categorieProject = new CategoryProject();


			 Optional<CategoryProject>  categorieBdd = _categoryProjectRepository.findByNom(listCategorie[i]);
	 ...
	 String[] listPorteProject = {"moi-même","mon association","mon entreprise"};
			...
			for(int i=0; i<listPorteProject.length; i++) {
				...
				PorteProject porteProject = new PorteProject();


				 Optional<PorteProject>  porteProjectBdd = _porteProjectRepository.findByNom(listPorteProject[i]);
